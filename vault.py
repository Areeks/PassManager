...
def previev_stored_passwords(master_key):
    ...
    for r in records:
        ...
        platform = r.split()[0] + separator * (platform_max_width - len(r.split()[0])) + ' | '
        login = r.split()[1] + separator * (login_max_width - len(r.split()[1])) + ' | '
        password = r.split()[2] + separator * (password_max_width - len(r.split()[2]))
...
def store_password(master_key, password=None):
    ...
    if password is None:
        ...
        print(f"\nProvide platrofm and login for the following password: {password}")
        str_platform = ''.join(input(prompt[0]).split())
        str_login = ''.join(input(prompt[1]).split())
    ...
    print("Record was added to Vault!")
...
def generate_new_password(master_key):
    ...
    while True:
        ...
        n_characters = input_as_digit(input(prompt[2]), prompt[2], 0, 10)
        ...
        str_special_chars = ''.join([random.choice(string.punctuation) for _ in range(n_characters)])
        ...
        print(
            "1. Re-generate.\n"
            "2. Use it for stroing a new record in the Vault.\n"
...
def main_menu(master_key):
    while True:
        ...
        if user_input == '1':
            ...
            previev_stored_passwords(master_key)
            continue
        elif user_input == '4':
            print("Your passwords are veeery safe :) Have a nice day!")
            exit()
        else:
            print("Bad choice!")
...
print("\n Greetins Master! Please provide the Master Key to proceded!")
